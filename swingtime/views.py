...
from swingtime.models import Event, Occurrence, EventType
...
class EventTypeCreateView(CreateView):
    model = EventType
    fields = ['abbr', 'label', 'tracker_url', 'point_person', 'users', 'related_group', 'status', 'description']
    success_url = '/karate'
    ...
    def get_success_url(self):
        ...
        if self.object:
            self.object.save_author(self.request)
        """

        return super(EventTypeCreateView, self).get_success_url()

#add_event_tpye = EventTypeCreateView.as_view()
def add_event_tpye(
    request,
    template='swingtime/event_type_form.html',
    event_type_form_class=forms.EventTypeForm,
):
    dtstart = None
    if request.method == 'POST':
        event_type_form = event_type_form_class(request.POST, initial={'request': request})
        if event_type_form.is_valid():
            event_type = event_type_form.save()
            #return http.HttpResponseRedirect(event_type.get_absolute_url())
            return month_view(request, '2017', '01')
        ...
        dtstart = dtstart or datetime.now()
        event_type_form = event_type_form_class(initial={'request': request})

    return render(
        request,
        template,
        {'dtstart': dtstart, 'event_type_form': event_type_form}
    )


def list_event_types(
        request,
        template='swingtime/event_type_list.html',
        events=None,
        **extra_context
):
    '''
    ...
    extra_context['event_types'] = event_types
    extra_context['request'] = request
    return render(request, template, extra_context)


def event_type_detail(
        request,
        template='swingtime/event_type_detail.html',
        slug=None,
        **extra_context
):
    '''
    ...
    '''
    #if slug is None:
    event_type = EventType.objects.filter(abbr=slug)

    extra_context['event_type'] = event_type[0]
    return render(request, template, extra_context)


# -------------------------------------------------------------------------------
def event_listing_for_event_type(
        request,
        template='swingtime/event_list_for_event_type.html',
        eventtype=None,
        **extra_context
):
    '''
    ...
    if eventtype is not None:
        events = Event.objects.filter(event_type__label=eventtype)
    else:
        events = None

    extra_context['events'] = events
    extra_context['event_type'] = eventtype
    return render(request, template, extra_context)