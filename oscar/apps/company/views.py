...
class CatalogueView(TemplateView):
    ...
    template_name = 'company/browse.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx[self.context_object_name] = Company.objects.all()
        #ctx.update(search_context)
        return ctx


class CompanyCategoryView(TemplateView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        return super(CompanyCategoryView, self).get(request, *args, **kwargs)
    ...
    def get_category(self):
        if 'pk' in self.kwargs:
            ...
            slugs = concatenated_slugs.split(Company._slug_separator)
            try:
                last_slug = slugs[-1]
            except IndexError:
                raise Http404
            else:
                for category in Company.objects.filter(slug=last_slug):
    ...
    def get_context_data(self, **kwargs):
        context = super(CompanyCategoryView, self).get_context_data(**kwargs)
