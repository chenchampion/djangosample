...
class EventForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kws):
        ...
        self.request = self.initial.get('request', None)
        self.fields['description'].required = False
        self.fields['event_type'].required = False

    def save(self, *args, **kws):
        obj = super(EventForm, self).save(*args, **kws)
...
class EventTypeForm(forms.ModelForm):
    ...
    # ---------------------------------------------------------------------------
    def __init__(self, *args, **kws):
        initial = kws.get('initial', None)
        self.request = None
        if initial:
            # self.request = kws['initial'].get('request', None)
            self.request = initial.get('request', None)

        super(EventTypeForm, self).__init__(*args, **kws)
        self.fields['description'].required = False
        if (self.request != None):
            group_name = self.request.user.groups.all()[0]
            self.fields['point_person'].queryset = CustomUser.objects.filter(groups__name=group_name)
            self.fields['users'].queryset = CustomUser.objects.filter(groups__name=group_name)
