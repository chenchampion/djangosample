...
from oscar.apps.dashboard.project import bforms
...
class OrderDetailView(DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == 'POST':
            if request.POST.has_key('createproject'):
                createform = bforms.CreateProjectForm(request.user, request.POST)
                if createform.is_valid():
                    project = createform.save_linked_project()
                    if(project and self.object):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['createform'] = bforms.CreateProjectForm()
