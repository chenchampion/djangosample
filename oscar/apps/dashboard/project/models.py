...
class Project(models.Model):
    ...
    owner = models.ForeignKey(AUTH_USER_MODEL, default="")
    ...
    def get_absolute_url(self):
        ...
        url = reverse(
            ...
            kwargs={'project_name': self.shortname})
        return url
    ...
    def new_tasks(self):
        ...
        return self.task_set.all().order_by('-created_on')
    ...
    def tasks_structure(self):
        overdue_tasks = self.task_set.filter(expected_end_date__lt = datetime.datetime.today())
        all_tasks = self.task_set.all().order_by('-created_on')
        if(all_tasks == None):
            return None
        parent_tasks = []
        sub_tasks = {}
        for task in all_tasks:
            if(not task.parent_task_num):
                parent_tasks.append(task)
                overdue = False
                if(task in overdue_tasks):
                    overdue = True
                sub_tasks.update({task.number:{'parent':task, 'overdue':overdue, 'sub':[]}})
        ...
        for task in all_tasks:
            if(task.parent_task_num):
                ...
                sub_task = sub_tasks.get(task.parent_task_num)
                sub_task['sub'].append({'task':task, 'overdue': overdue})
...
class Task(models.Model):
    ...
    user_responsible = models.ForeignKey(AUTH_USER_MODEL, default="")
    ...
    created_by = models.ForeignKey(AUTH_USER_MODEL, default="", related_name = '%(class)s_created_tasks')
    last_updated_by = models.ForeignKey(AUTH_USER_MODEL, default="", related_name = '%(class)s_updated_tasks')
    #Versioning
    ...
    def num_items(self):
        ...
        num = 0
        try:
            num = self.taskitem_set.all().count()
        except:
            return 0
    ...
    def get_absolute_url(self):
        ...
        url = reverse(
            'dashboard:task_details',
            kwargs={'project_name': self.project.shortname, 'task_num': self.number})
        return url
...
class TaskItem(models.Model):
    ...
    created_on = models.DateTimeField(auto_now_add = 1)
    created_by = models.ForeignKey(AUTH_USER_MODEL, default="", related_name = '%(class)s_created_taskitems')
    last_updated_by = models.ForeignKey(AUTH_USER_MODEL, default="",  related_name = '%(class)s_updated_taskitems')
    is_deleted = models.BooleanField(default = False)
    #Versioning
