...
from models import Project, SubscribedUser, options, InvitedUser, unit_choices, Task, TaskItem, WikiPage, WikiPageRevision, Notice, TodoList, ProjectFile, ProjectFileVersion, LinkedProject
...
class CreateProjectForm(MarkedForm):
    ...
    def save_linked_project(self):
        project = LinkedProject(name = self.cleaned_data['name'], shortname=self.cleaned_data['shortname'])
        project.owner = self.user
        project.start_date = self.cleaned_data['start_date']
        project.save()
        subscribe = SubscribedUser(user = self.user, project = project, group = 'Owner')
...
class EditTaskForm(CreateTaskForm):
    ...
    #    required=False, label=_("Date from"), widget=DatePickerInput,
    #    help_text='When did this task start?')
    actual_end_date = DojoDateField(required = False, help_text='When did this task end?')
    #actual_start_date = forms.DateField(
    #    required=False, label=_("Date to"), widget=DatePickerInput,
    #    help_text='When did this task end?')
    is_complete = forms.BooleanField(required = False, help_text = 'Is this task complete?')
