...
from helpers import *
from models import Project, SubscribedUser, InvitedUser, TodoItem, Notice, TodoList, Log
import bforms
...
def index(request):
    ...
    payload = {'request': request, 'user':request.user, 'register_form': register_form, 'login_form': login_form}
...
def dashboard(request):
    ...
    payload = {'request':request, 'user':request.user, 'subs': subs, 'createform': createform, 'invites': invites}
...
class ProjectMainPageView(generic.ListView):
    """
    ...
    """
    template_name = 'dashboard/project/mainpage.html'
    form_class = bforms.CreateProjectForm

    def get_queryset(self):
        if self.request.method == 'GET':
            if self.request.GET.has_key('createproject'):
                createform = bforms.CreateProjectForm(self.request.user, self.request.GET)
                if createform.is_valid():
                    createform.save()
                    return HttpResponseRedirect('.')
    ...
    def get_context_data(self, **kwargs):
        ctx = super(ProjectMainPageView, self).get_context_data(**kwargs)
        if(self.request):
            ctx['current_user'] = self.request.user
            ctx['subs'] = self.request.user.subscribeduser_set.all()
...
def project_details(request, project_name):
    ...
    #new_tasks = project.new_tasks()
    group_tasks = project.tasks_structure()
    ...
    if request.GET.get('csv', ''):
        ...
        return response
    '''
    payload = {'request':request, 'user':request.user,'project': project, 'inviteform': inviteform, 'taskform': taskform, 'group_tasks':group_tasks.values(), 'access': access, 'subs':subs}
