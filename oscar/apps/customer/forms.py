...
class EmailUserCreationForm(SignupForm):
    redirect_url = forms.CharField(
        widget=forms.HiddenInput, required=False)
    ...
    def save(self, commit=True):
        user = super(EmailUserCreationForm, self).save(request=self.request)
        user.set_password(self.cleaned_data['password1'])

        if commit:
            user.save()
            dashboard_access_perm = Permission.objects.get(
                codename='dashboard_access', content_type__app_label='customer')
...
class UserForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = existing_user_fields(['first_name', 'last_name',  'email', 'groups', 'isCreator', 'title', 'company', ])
